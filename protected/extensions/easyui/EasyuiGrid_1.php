...
class EasyuiGrid extends EasyuiWidget{
	...
	public function run(){
                echo CHtml::tag('table',array('id'=>'grid_wrap'),false,true);
		$htmlOptions='';
		...
		if(isset($this->toolbar_show['add'])){
          ...
          //          echo '<iframe src="'.$this->toolbar_show['add']['href'].'" marginheight=0 marginwidth=0  scrolling="none" framespacing=0 frameborder=0 width="100%" height="99%"></iframe>';
			...
			",CClientScript::POS_END);
                     echo CHtml::tag('div',array('class'=>'datagrid-btn-separator'),false,true);
                    //    $toolbars[]='-';
		}
		...
		if(isset($this->toolbar_show['multiple'])){
			...
			$this->cs->registerScript('multipleRow',"
					...
					function multipleRow(){
                                            var m_tpl_url='".Yii::app()->createUrl('public/gridMultiple',array('model'=>$this->model,'href'=>$this->toolbar_show['multiple']['href'],'formconfig'=>$this->toolbar_show['multiple']['formTpl']))."';
